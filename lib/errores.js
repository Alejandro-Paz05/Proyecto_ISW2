...
// Un marco de pila de Chrome, Edge y Node: "    at funcion (archivo:1:2)".
const MARCO_V8 = /^\s*at\s+(?:(\S+)\s+\()?(.+?):\d+:\d+\)?\s*$/;
// Un marco de pila de Firefox y Safari: "funcion@archivo:1:2".
const MARCO_FIREFOX = /^\s*([^@\s]*)@(.+?):\d+:\d+\s*$/;
...
export function lugarDelError(pila) {
  for (const linea of String(pila ?? '').split('\n')) {
    const marco = linea.match(MARCO_V8) ?? linea.match(MARCO_FIREFOX);
    if (!marco) continue;

    const [, funcion, archivo] = marco;
