...
/**
 * Registra este navegador en su servicio de avisos.
 *
 * El error que devuelve el navegador cuando eso falla es "Registration failed
 * - push service error": en inglés, sin decir por qué ni qué hacer. La causa
 * casi siempre es la misma y tiene arreglo en dos clics: Brave trae apagado el
 * servicio de push de Google, y sin él ningún sitio puede suscribirse.
 */
async function suscribirse(registro) {
  try {
    ...
    if (/push service|registration failed/i.test(error?.message ?? '')) {
      throw new Error(
        'Este navegador no pudo conectarse al servicio de avisos. Si usás Brave, ' +
          'activá "Usar servicios de Google para mensajería push" en brave://settings/privacy ' +
          'y reiniciá el navegador. En Chrome, Edge o Firefox funciona sin configurar nada.'
      );
    }
...
export async function activarAvisos() {
  ...
  const suscripcion =
    (await registro.pushManager.getSubscription()) ?? (await suscribirse(registro));
