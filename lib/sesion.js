...
// Se reexportan para que las rutas de API sigan pidiéndoselos a este módulo,
// que es donde vive todo lo de permisos. Las páginas que los necesitan al
// dibujar los toman de lib/roles, que no arrastra nada del servidor.
export { ROLES, PANEL_TIENDA, PORTAL_SISTEMA, puedeEscribir, destinoSegunRol };
...
/** El rol de una cuenta, leído de la base con la clave secreta. */
export async function rolDeUsuario(idUsuario) {
...
/**
 * La cuenta de Supabase de la petición, sin rol. Null si no hay.
 *
 * Para las rutas que solo necesitan saber quién es, como la
 * retroalimentación: no pagan la consulta del rol ni fallan si esa consulta
 * falla.
 */
export async function cuentaDeSesion(req, res) {
...
export async function leerSesion(req, res) {
  const usuario = await cuentaDeSesion(req, res);

  if (usuario) {
    return {
      id: usuario.id,
      email: usuario.email ?? null,
      rol: await rolDeUsuario(usuario.id),
      origen: 'supabase'
    };
  }
...
/** Si una ruta es del propio sitio: empieza por /akaristudio y nada más. */
export function esRutaPropia(ruta) {
  return typeof ruta === 'string' && /^\/akaristudio(\/|\?|$)/.test(ruta);
}
...
export function destinoSeguro(volver, rol) {
  return esRutaPropia(volver) ? volver : destinoSegunRol(rol);
}
